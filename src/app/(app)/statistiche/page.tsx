  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Analisi preventivi</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        ...
        <Kpi
          label="Operatori"
          value={data.totali.operatori}
          icon={UserCog}
          iconBg="bg-purple-100"
          iconColor="text-purple-600"
        />
        <Kpi
          label="Tempo medio accett."
          value={tempoMedio}
          icon={Timer}
          iconBg="bg-gray-100"
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Preventivi per mese (ultimi 12 mesi)</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex items-end gap-2 h-48">
            {data.perMese.map((m) => (
              ...
              >
                ...
                <div
                  className="w-full rounded-t bg-sky-500 hover:bg-sky-600 transition-colors min-h-[2px]"
            ...
            <div className="space-y-3">
              {data.perStato.map((s) => (
                <Link
                  ...
                  className="flex items-center justify-between p-2.5 rounded-lg border border-gray-100 hover:border-sky-200 hover:bg-sky-50 transition-colors"
                >
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <UserCog className="h-4 w-4" /> Preventivi per operatore
            </CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <p className="text-xs text-gray-400">
        Legenda stati:{" "}
        {data.perStato.map((s) => STATO_PREVENTIVO_LABELS[s.stato]).join(" · ")}
      </p>
    </div>
  );
}
